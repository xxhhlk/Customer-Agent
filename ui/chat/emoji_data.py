...
# 名称 → emoji 字符，仅用于「面板显示 + 气泡本地预览」（本地预览，与发送内容无关）。
#
# 这是一份「标准微信表情命名 → Unicode 字形」的显示映射表，覆盖微信表情体系
# 的绝大多数名称（PDD 表情命名空间 = 微信体系 + PDD 自有扩展）。
# 用途：用户按 §6 从拼多多导入官方全量后，面板/气泡里能显示成 emoji 符号，
#       而不是干巴巴的中文原名。
#
# 注意：
# - 这「不是」PDD 官方表情目录，也不会污染默认分组（默认面板仍只有 verified 14 个）。
# - 映射可能有个别与 PDD 实际图片不一致（仅本地预览差异，发送内容不受影响），属计划
#   已标注的低风险。
# - 查不到映射的名字（如 PDD 独有扩展）在渲染时保留原中文名，仍可辨认。
EMOJI_UNICODE: dict[str, str] = {
    # 已实测 PDD 可渲染（verified，共 14 个）
    "玫瑰": "🌹",
    "抱拳": "🙏",
    "微笑": "😊",
    "点赞": "👍",
    ...
    "裂开": "💥",
    "苦涩": "😖",
    # PDD 店铺/客服扩展（用户从官方导入的常见名，无标准微信对应，按语义就近映射）
    "大爱": "💗",
    "嘻嘻": "😆",
    "囧": "😅",
    "愉快": "😄",
