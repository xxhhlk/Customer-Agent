...
class CozeRateLimiter:
    ...
    def configure(self, window_size: float = None, max_requests: int = None):
        ...
        with self._lock:
            if window_size is not None:
                ...
                self.logger.info(f"限流器窗口大小已更新为 {window_size} 秒")
            if max_requests is not None:
                self._max_requests = max_requests
                self.logger.info(f"限流器最大请求数已更新为 {max_requests}")
    ...
    def is_rate_limited(self, from_uid: str) -> bool:
        ...
        with self._lock:
            ...
            if user is None:
                ...
                self.logger.info(f"用户 {from_uid} 首次请求，创建新窗口")
                return False
            ...
            if elapsed >= user.window_size:
                ...
                self.logger.info(f"用户 {from_uid} 窗口已过期（{elapsed:.0f}s），重置计数")
                return False
            ...
            self.logger.debug(f"用户 {from_uid} 计数 {user.count}/{self._max_requests}")
            return False
    ...
    def cleanup_expired_users(self, max_age_hours: float = 24) -> int:
        ...
        if cleaned > 0:
            self.logger.info(f"限流器清理了 {cleaned} 个过期用户记录")
