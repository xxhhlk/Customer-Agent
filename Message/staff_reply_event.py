...
class StaffReplyEventManager:
    ...
    def start_waiting(self, from_uid: str) -> str:
        ...
        with self._lock:
            ...
            logger.debug(f"[StaffReplyEvent] 开始等待客服回复: {from_uid}, 事件ID: {event_id}, 当前等待数: {len(self._waiting_events[from_uid])}")
            return event_id
        ...
        with self._lock:
            ...
            for i, event_info in enumerate(event_list):
                if event_info["event_id"] == event_id:
                    del event_list[i]
                    logger.debug(f"[StaffReplyEvent] 停止等待客服回复: {from_uid}, 事件ID: {event_id}, 剩余等待数: {len(event_list)}")
                    break
    ...
    def notify_staff_reply(self, from_uid: str) -> bool:
        ...
        with self._lock:
            ...
            for event_info in event_list:
                ...
                if event is None:
                    # 事件对象还未创建，说明等待还没开始
                    logger.debug(f"[StaffReplyEvent] 买家 {from_uid} 的事件 {event_info['event_id']} 对象未创建，跳过通知")
                    continue
            ...
            if notified_count > 0:
                logger.info(f"[StaffReplyEvent] 人工客服已回复，通知 {from_uid} 的 {notified_count} 个等待事件")
    ...
    async def wait_for_staff_reply(self, from_uid: str, event_id: str, timeout: float) -> bool:
        ...
        with self._lock:
            ...
            if not event_info:
                logger.warning(f"[StaffReplyEvent] 事件 {event_id} 不存在，可能已经被清理")
                return False
        ...
        logger.debug(f"[StaffReplyEvent] 开始等待 {timeout}秒: {from_uid}, 事件ID: {event_id}")
        
        try:
            # 等待事件或超时
            await asyncio.wait_for(event.wait(), timeout=timeout)
            logger.info(f"[StaffReplyEvent] 人工客服已回复: {from_uid}, 事件ID: {event_id}")
            return True
        except asyncio.TimeoutError:
            logger.debug(f"[StaffReplyEvent] 等待超时 {timeout}秒: {from_uid}, 事件ID: {event_id}")
            return False
        finally:
            # 清理当前事件
